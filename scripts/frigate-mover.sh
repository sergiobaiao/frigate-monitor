...
move_media_older_than_day() {
    ...
    local rel_dir dir_to_create last_rel_dir=""
    ...
    while IFS= read -r -d '' rel_path; do
        ...
        if [[ "$DRY_RUN" != "1" ]]; then
            ...
            if [[ "$rel_dir" == "." ]]; then
                ...
                dir_to_create="$dst/$rel_dir"
            fi
            if [[ "$dir_to_create" != "$last_rel_dir" ]]; then
                mkdir -p "$dir_to_create" 2>/dev/null || true
                chown "${FRIGATE_UID}:${FRIGATE_GID}" "$dir_to_create" 2>/dev/null || true
                last_rel_dir="$dir_to_create"
            fi
        fi
    ...
    if (( total > 0 )) && [[ "$DRY_RUN" != "1" ]]; then
        ...
        if run_rsync "$label:batch" -a --bwlimit="$BWLIMIT" --chown="${FRIGATE_UID}:${FRIGATE_GID}" \
            ...
            log_warn "$LOG_TAG" "Falha no batch de $label, iniciando fallback por arquivo"
            while IFS= read -r -d '' rel_path; do
                abs_path="$src/$rel_path"
                dest_file="$dst/$rel_path"
                ...
                chown "${FRIGATE_UID}:${FRIGATE_GID}" "$dest_dir" 2>/dev/null || true

                err_file="$(mktemp)"
                rc=0
                rsync -a --bwlimit="$BWLIMIT" --chown="${FRIGATE_UID}:${FRIGATE_GID}" \
                    --ignore-existing --ignore-missing-args "$abs_path" "$dest_file" 2>"$err_file" || rc=$?

                if (( rc == 0 )); then
                    ((++fallback_synced))
                else
                    err_tail="$(tail -n 5 "$err_file" | tr '\n' '; ')"
                    [[ -z "$err_tail" ]] && err_tail="sem detalhes de stderr"
                    log_warn "$LOG_TAG" "Fallback falhou $label:$rel_path (exit=$rc, detalhes=$err_tail)"
                    ((++fallback_failed))
                fi
            ...
            log "$LOG_TAG" "Fallback $label: copiados=$fallback_synced falhas=$fallback_failed pulados=$fallback_skipped"
        fi
        ...
        while IFS= read -r -d '' rel_path; do
            ...
            if [[ -e "$dest_file" ]]; then
                ((++synced))
                size_b="$(stat -c%s "$dest_file" 2>/dev/null || echo 0)"
                synced_bytes=$((synced_bytes + size_b))
            else
                ((++missing_after))
            fi
        done < "$list_file"

        if (( batch_ok == 0 && missing_after > 0 )); then
            log_warn "$LOG_TAG" "$label ainda com lacunas apos fallback: faltando=$missing_after de $total"
        fi
    fi
...
# Guard rails para evitar contencao com prune quando HD esta pressionado.
if [[ "$MODE" == "file" || "$MODE" == "incremental" ]]; then
    current_minute="$(date +%M)"
    if ((10#$current_minute >= 40 && 10#$current_minute <= 49)); then
        log "$LOG_TAG" "Janela reservada ao prune (:40-:49), pulando execução (mode=$MODE)"
        exit 0
    fi
fi

hd_usage_now="$(get_disk_usage_pct "$HD_MOUNT" || echo 0)"
if [[ "$MODE" != "emergency" ]] && [[ "$hd_usage_now" =~ ^[0-9]+$ ]] && (( hd_usage_now >= 95 )); then
    log "$LOG_TAG" "HD em ${hd_usage_now}% (>=95%), pulando $MODE para priorizar prune"
    exit 0
fi

...
# Guard rails para evitar contencao com prune e pressao em disco cheio.
current_minute="$(date +%M)"
if ((10#$current_minute >= 40 && 10#$current_minute <= 49)); then
  log "$LOG_TAG" "Janela reservada ao prune (:40-:49), pulando reconcile"
  exit 0
fi

hd_usage_now="$(get_disk_usage_pct "$HD_MOUNT" || echo 0)"
if [[ "$hd_usage_now" =~ ^[0-9]+$ ]] && (( hd_usage_now >= 95 )); then
  log "$LOG_TAG" "HD em ${hd_usage_now}% (>=95%), pulando reconcile para priorizar prune"
  exit 0
fi
